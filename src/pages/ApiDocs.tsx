...
const ApiDocs = () => {
    ...
    >
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        <motion.section
          ...
          variants={itemVariants}
        >
          <h2 className="text-3xl font-bold text-vf-purple mb-4 flex items-center">
          ...
          <div className="bg-vf-gray/30 p-4 rounded-md text-vf-blue font-mono break-all flex justify-between items-center">
            ...
            <VisuaForgeButton variant="ghost" size="sm" onClick={() => copyToClipboard(webhookUrl)}>
              Copier
            </VisuaForgeButton>
          </div>
        </motion.section>
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-vf-blue mb-2">Requête POST</h3>
          <pre className="bg-vf-gray/30 p-4 rounded-md text-white font-mono text-sm overflow-x-auto">
            <code>
  ...
  -d '{
    ...
    "style": "futuristic"
            ...
            <VisuaForgeButton variant="ghost" size="sm" className="float-right" onClick={() => copyToClipboard(`curl -X POST ${webhookUrl} -H "Content-Type: application/json" -d '{"mode": "generate", "idea": "un robot sur la lune, style cyberpunk, néon", "style": "futuristic"}'`)}>
              Copier
            </VisuaForgeButton>
          </pre>
          <h3 className="text-xl font-semibold text-vf-blue mt-6 mb-2">Réponse (JSON)</h3>
          <pre className="bg-vf-gray/30 p-4 rounded-md text-white font-mono text-sm overflow-x-auto">
            <code>
              {`{
  "status": "success",
  ...
  "message": "Image générée avec succès."
            ...
            <VisuaForgeButton variant="ghost" size="sm" className="float-right" onClick={() => copyToClipboard(`{"status": "success", "imageUrl": "https://public.dyad.sh/assets/placeholder.svg", "message": "Image générée avec succès."}`)}>
              Copier
            </VisuaForgeButton>
          </pre>
        </motion.section>
        ...
        >
          ...
          <h3 className="text-xl font-semibold text-vf-blue mb-2">Requête POST (multipart/form-data)</h3>
          <pre className="bg-vf-gray/30 p-4 rounded-md text-white font-mono text-sm overflow-x-auto">
            <code>
              {`curl -X POST ${webhookUrl} \\
  ...
  -F "image=@/chemin/vers/ton/image.png"`}
            ...
            <VisuaForgeButton variant="ghost" size="sm" className="float-right" onClick={() => copyToClipboard(`curl -X POST ${webhookUrl} -H "Content-Type: multipart/form-data" -F "mode=edit" -F "instruction=ajoute un fond spatial" -F "image=@/chemin/vers/ton/image.png"`)}>
              Copier
            </VisuaForgeButton>
          </pre>
          <h3 className="text-xl font-semibold text-vf-blue mt-6 mb-2">Réponse (JSON)</h3>
  ...
  "message": "Image modifiée avec succès."
            ...
            <VisuaForgeButton variant="ghost" size="sm" className="float-right" onClick={() => copyToClipboard(`{"status": "success", "imageUrl": "https://public.dyad.sh/assets/placeholder.svg", "message": "Image modifiée avec succès."}`)}>
              Copier
            </VisuaForgeButton>
          </pre>
        </motion.section>
        ...
        <motion.section
          ...
          variants={itemVariants}
        >
          <h2 className="text-3xl font-bold text-vf-blue mb-4">Besoin d'aide ?</h2>
          <p className="text-lg text-vf-gray mb-6">
            Consultez notre documentation complète ou contactez notre support pour toute question.
          </p>
          <VisuaForgeButton asChild>
            <Link to="/contact" className="text-lg px-6 py-3">
